...
var buys_url = '/shares-gradle/analyse/getTotalBuys.do';
var analyse_buy_url = '/shares-gradle/analyse/getDayDatas.do';
$(function() {
	$.ajax({
        type: "GET",
        url: buys_url + "?date=" + new Date(),
        dataType: "json",
	...
	$("#buys_div .buysTable").delegate("tr", "click", function () {
		showChart($(this).find(".shareCode").html(), 10, $(this).find(".javaBuyTime").val());
	...
	$("#buys_div .buysTable").delegate("tr", "mouseover", function () {
		$(this).parent().find("tr").removeClass("focusBackColor");
...
function formatTable(data) {
	...
	if (isNotNull(buys)) {
		for (var i =0; i < buys.length; i++) {
			if (isNotNull(buys[i].buyPrice) && isNotNull(buys[i].sellPrice)) {
				profit = (buys[i].sellPrice / buys[i].buyPrice - 1) * 100;
			} else {
				profit = -99;
			}
			tempTr = $(".hideBuyTr").clone();
			tempTr.find(".shareCode").html(buys[i].shareCode);
			tempTr.find(".shareName").html(buys[i].shareName);
			tempTr.find(".buyTime").html(getTimeStrFormJava(buys[i].buyTime));
			tempTr.find(".buyPrice").html(buys[i].buyPrice.toFixed(2));
			if (isNotNull(buys[i].sellPrice)) {
				tempTr.find(".sellPrice").html(buys[i].sellPrice.toFixed(2));
			}
			tempTr.find(".profit").html(profit.toFixed(2));
			tempTr.find(".javaBuyTime").val(buys[i].buyTime);
...
function showChart(shareCode, day, buyTime) {
	if (isNotNull(shareCode)) {
		var endTime = formatNumber(buyTime) + 1000 * 60 * 60 * 24 * 3;
		$.ajax({
	        type: "GET",
	        url: analyse_buy_url + "?shareCode=" + shareCode + "&endTime=" + endTime + "&day=" + day + "&date=" + new Date(),
	        dataType: "json",
	        success: function(data){
	        	...
	        	if (isNotNull(days)) {
	        		...
	        		var coordinate = 0;
	        		for (var i =0; i < days.length; i++) {
	        			timeSpan = formatNumber(days[i].updateTime - buyTime);
	        			coordinate = days.length - 1 - i;
	        			maxPriceData.push([coordinate,
	        			                             formatNumber(days[i].maxPrice) ]);
	        			startPriceData.push([coordinate,
  			                             formatNumber(days[i].todayStart) ]);
	        			endPriceData.push([coordinate,
	  			                             formatNumber(days[i].todayEnd) ]);
	        			maxDealNumData.push([coordinate,
